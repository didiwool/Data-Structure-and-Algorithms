...
{
    //Create a new node with the value of v.
    struct IntListNode *new = newIntListNode(v);
    /*If the list is empty or v is largest,
    can use InListInsert directly.*/
	if (L->first == NULL || v >= L->last->data)
        IntListInsert(L, v);
    //If v is smallest, insert in the head of list.
    ...
    /*
    I think it not necessary to consider the case "second smallest"
    or "second largest". Add these two cases into the case "middle of the list".
    */
    else {
        struct IntListNode *curr = L->first;
        for (curr = L->first;curr->next != L->last;curr = curr->next) {
            if (v <= curr->next->data) break;
        }
        new->next = curr->next;
        curr->next = new;
        L->size ++;
